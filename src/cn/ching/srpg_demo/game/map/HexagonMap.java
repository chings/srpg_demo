...
public class HexagonMap<T> implements GeometryMap, Iterable<TileVisit<T>> {
	...
	public Rect getRect() {
		if(rect == null) {
			rect = new Rect(
				...
				origin.getX() + (rows > 1 ? columns * origin.getWidth() + origin.getHalfWidth() : columns * origin.getWidth()),
	...
	public Geometry coordinate(int[] position) {
		...
		if(hexagon == null) {
			hexagon = new Hexagon(
				position[0] * origin.getWidth() + position[1] * origin.getHalfWidth() + origin.getX(),
				position[1] * origin.getBaseHeight() + origin.getY(),
				origin.getHalfWidth(),
				origin.getMidHeight(),
				origin.getEndHeight());
	...
	public int[] decoordinate(int x, int y) {
		...
		int yOdd0 = Math.round((float)origin.getEndHeight() / (float)origin.getHalfWidth() * xOdd);
		if((yBlock & 1) == 0) {	//even row
			if(yOdd < origin.getEndHeight() - yOdd0) {	//upleft
				m0 = xBlock - 1;
				n = yBlock - 1;
			} else if(yOdd < yOdd0 - origin.getEndHeight()) {	//upright
				m0 = xBlock;
				n = yBlock - 1;
			} else {	//down
				m0 = xBlock;
			...
			if(xOdd < origin.getHalfWidth()) {	//left half
				if(yOdd < yOdd0) {	//up
					m0 = xBlock;
					n = yBlock - 1;
				} else {	//down
				...
				if(yOdd < origin.getEndHeight() + origin.getEndHeight() - yOdd0) {	// up
					m0 = xBlock;
					n = yBlock - 1;
				} else {	//down
					m0 = xBlock;
	...
	static int[][] orientationsByOffset = {
		{LEFT,UPLEFT,UPRIGHT},
		{LEFT,NONE,RIGHT},
	...
	static int[][] offsetsByOrientation = {
		{-1, 0},	//LEFT
		{-1, 1},	//DOWNLEFT
		{0, -1},	//UPLEFT
		{0, 0},		//NONE
	...
	public Iterable<TileVisit<T>> part(Rect inRect) {
